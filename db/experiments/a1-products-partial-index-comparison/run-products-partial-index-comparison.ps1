...
$sqlPath = Join-Path $experimentDir "products_partial_index_comparison.sql"
...
try {
    foreach ($targetTable in $profiles) {
        ...
        $outputFile = Join-Path $resultDir "$($targetTable)_partial_index_comparison_$timestamp.txt"

        Write-Host "Running partial index comparison for target_table=$targetTable"
