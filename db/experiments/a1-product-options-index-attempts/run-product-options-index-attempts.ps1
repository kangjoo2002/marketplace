...
$sqlPath = Join-Path $experimentDir "product_options_index_attempts.sql"
...
try {
    foreach ($productsTable in $profiles) {
        ...
        $outputFile = Join-Path $resultDir "$($productsTable)_product_options_index_attempts_$timestamp.txt"

        Write-Host "Running product_options index attempts for products_table=$productsTable product_options_table=$productOptionsTable"
