...
$sqlPath = Join-Path $experimentDir "join_keyset_pagination.sql"
...
try {
    foreach ($productsTable in $profiles) {
        ...
        $outputFile = Join-Path $resultDir "$($productsTable)_join_keyset_pagination_$timestamp.txt"

        Write-Host "Running JOIN/EXISTS keyset pagination comparison for products_table=$productsTable product_options_table=$productOptionsTable"
