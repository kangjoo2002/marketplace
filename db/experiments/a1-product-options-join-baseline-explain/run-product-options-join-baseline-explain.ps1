...
$sqlPath = Join-Path $experimentDir "product_options_join_baseline_explain.sql"
...
try {
    foreach ($productsTable in $profiles) {
        ...
        $outputFile = Join-Path $resultDir "$($productsTable)_product_options_join_baseline_explain_$timestamp.txt"

        Write-Host "Running product_options JOIN baseline EXPLAIN for products_table=$productsTable product_options_table=$productOptionsTable"
