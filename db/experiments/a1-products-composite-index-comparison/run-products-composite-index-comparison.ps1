...
$sqlPath = Join-Path $experimentDir "products_composite_index_comparison.sql"
...
try {
    foreach ($targetTable in $profiles) {
        ...
        $outputFile = Join-Path $resultDir "$($targetTable)_composite_index_comparison_$timestamp.txt"

        Write-Host "Running composite index comparison for target_table=$targetTable"
